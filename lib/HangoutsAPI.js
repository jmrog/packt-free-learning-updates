...
function sendHangoutsMessage(receivedData) {
    ...
    // Unfortunately, Hangouts doesn't allow directly inserting links, so the
    // best we can do is append the URL, which Hangouts will render clickable.
    const message = `Today's free Packt title is: ${title}. ${PACKT_URL}`;
    ...
    return new Promise((resolve) => {
        // FIXME: This is somewhat inelegant, but I don't fully grok XMPP yet.
        // It appears that any occurrence of a stanza event is an indication
        // that the message was sent to the server, so we'll use that for now
        // as an indication that we can exit (we can't exit immediately because
        // some part of sending the message is apparently asynchronous).
        this.hangoutsBot.connection.on('stanza', () => resolve(0)); // 0 is the exit code
...
function sendErrorNotification(error) {
    ...
    return new Promise((resolve) => {
        // See comment above about this process.
        this.hangoutsBot.connection.on('stanza', () => resolve(1)); // 1 is the exit code
        this.hangoutsBot.sendMessage(this.destination, message);
    });
}
